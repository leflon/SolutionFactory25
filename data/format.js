...
// Tables
db.exec(`CREATE TABLE IF NOT EXISTS Stops(id INTEGER, name TEXT, plain_name TEXT, line TEXT, is_terminal INTEGER, branch INTEGER, pos_x INTEGER, pos_y INTEGER)`);
...
while ((parsed = stopParser.exec(stops)) !== null) {
  ...
  const name = parsed[2];
  const plainName = normalize(name);
  ...
  insertStop.run(id, name, plainName, line, is_terminal ? 1 : 0, branch, parseInt(pos_x), parseInt(pos_y));
}
